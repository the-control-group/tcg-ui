...
const Icon = ({
	...
	svgs.keys().map(svg => {
		...
		if (svgName === name) {
			...
			image =
				require(`./svgs/${svgName}.svg`).default ||
				require(`./svgs/${svgName}.svg`);
		}
	});
...
Icon.propTypes = {
	...
	height: (props, propName, componentName) => {
		if (!props.width && !props.height && !props.size) {
			return new Error(
				`${componentName} requires either 'size' prop, or both 'height' and 'width' props`
			);
			...
			return new Error(
				`${componentName} requires 'height' to be of type string or number`
			);
		}
	},
	width: (props, propName, componentName) => {
		if (!props.width && !props.height && !props.size) {
			...
			return new Error(
				`${componentName} requires 'width' to be of type string or number`
			);
		}
	},
	size: (props, propName, componentName) => {
		if (!props.width && !props.height && !props.size) {
			...
			return new Error(
				`${componentName} requires 'size' to be of type string or number`
			);
		}
	},
